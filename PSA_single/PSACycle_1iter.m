function  [objectives, constraints, a, b, c, d, e] = PSACycle(vars, iso_params, material, type, N, x0)
...
%   Specify the parameters for the simulation
    ...
    % Input parameters - First convert iso_params b0 values to [Pa^-1]
%     iso_params_sort = [iso_params(:,1:2),iso_params(:,3:4)*1e-5,...
%         -iso_params(:,5:6),iso_params(:,7:8),iso_params(:,9:10)*1e-5,...
%         -iso_params(:,11:end)];
%     
    iso_params_sort = iso_params; 
    % Input parameters
    InputParams     = ProcessInputParameters(vars, iso_params_sort, material, N) ;
...
%         tic; 
        [t1, a] = ode23t(CoCPressurization_fxn, [0 tau_CoCPres], x0, opts1) ;
%         time_for_solver = [time_for_solver; toc];
...
%         tic; 
        [t2, b] = ode23t(Adsorption_fxn, [0 tau_ads], x10, opts2) ;
%         time_for_solver = [time_for_solver; toc]; 
...
%         tic; 
        [t3, c] = ode23t(HeavyReflux_fxn, [0 tau_HR], x20, opts3) ;
%         time_for_solver = [time_for_solver; toc]; 
        % Correct the output (clean up results from simulation)
...
%         tic;
        [t4, d] = ode23t(CnCDepressurization_fxn, [0 tau_CnCDepres], x30, opts4) ;
%         time_for_solver = [time_for_solver; toc]; 
...
%         tic; 
        [t5, e] = ode23t(LightReflux_fxn, [0 tau_LR], x40, opts5) ;
%         time_for_solver = [time_for_solver; toc]; 
...
%       
    %% Check CCS condition
        ...
        [cyclic_check, ~] = CCS_Check(a, b, c, d, e, t1, t2, t3, t4, t5) ;
        ...
        if CSS_states <= 1e-4 && abs(massBalance-1) <= 0.005 && cyclic_check == 1
            break
        end
%       
    end
%     disp(['average elapsed time: ', num2str(mean(time_for_solver)), ' seconds; total time: ', num2str(sum(time_for_solver))]);
...
%     radius_inner = sqrt((desired_flow.*cycle_time/gas_fed)/pi()) ;    % m
    ...
    end_of_function = toc;
    disp(['total cycle time: ', num2str(end_of_function)])
%   
end 
%   
%% Filter stored data 
...
%% Complementary Functions
    ...
    function [cyclic_check, cyclic_display] = CCS_Check(varargin)
        %CCS_Check: Check whether the PSA cylce has reached CCS
        %   CCS is defined as the mole fraction, CO_2 molar loading, and
        %   temperature experiences a change of less than 0.5% or 0.0001 over
        %   the dimensionless variable. In addition, the overall mass balance
        %   of the gas entering and leaving the column needs to be within 0.995
        %   and 1.005. Finally, for any cycle that required estimation of input
        %   parameters before the actual step (eg. Heavy Reflux cycles at
        %   atmospheric pressure supplied from depressurization steps), the
        %   change in the value from one cycle needs to be less than 0.5%
        %
        %% Calculate change in the state variables over the cycle
        
        rel_error_tolerance = 0.005 ;
        abs_error_tolerance = 0.001 ;
        
        initial_step = varargin{1}        ;
        final_step   = varargin{nargin/2} ;
        
        y_init = initial_step(1, N+4:2*N+3)     ;
        q_init = initial_step(1, 2*N+6:3*N+5)   ;
        T_init = initial_step(1, 4*N+10: 5*N+9) ;
        
        y_end = final_step(end, N+4:2*N+3)      ;
        q_end = final_step(end, 2*N+6:3*N+5)    ;
        T_end = final_step(end, 4*N+10: 5*N+9)  ;
        
        change_y = abs(y_end-y_init) ;
        change_q = abs(q_end-q_init) ;
        change_T = abs(T_end-T_init) ;
        
        max_error_y_array = zeros(2, N) ;
        max_error_q_array = zeros(2, N) ;
        max_error_T_array = zeros(2, N) ;
        
        max_error_y_array(1, :) = abs_error_tolerance ;
        max_error_q_array(1, :) = abs_error_tolerance ;
        max_error_T_array(1, :) = abs_error_tolerance ;
        
        max_error_y_array(2, :) = rel_error_tolerance*y_init ;
        max_error_q_array(2, :) = rel_error_tolerance*q_init ;
        max_error_T_array(2, :) = rel_error_tolerance*T_init ;
        
        max_error_y = max(max_error_y_array) ;
        max_error_q = max(max_error_q_array) ;
        max_error_T = max(max_error_T_array) ;
        
        error_y = change_y-max_error_y ;
        error_q = change_q-max_error_q ;
        error_T = change_T-max_error_T ;
        
        error = length(find(error_y >=0))+length(find(error_q >=0))+length(find(error_T >=0)) ;
    ...
    %% Calculate the mass balance
        ...
        [~, rec, mass_balance] = ProcessEvaluation(varargin{:}) ;
        
        if abs(mass_balance-1) <= 0.005
            mass_balance_check = 1 ;
        else
            mass_balance_check = 0 ;
        end
	%   
    %% Determine if CCS has been acheived or not
        
        if mass_balance_check*state_var_check == 1
            cyclic_check = 1 ;
        else
            cyclic_check = 0 ;
        end
    ...
    %% Create Display
        ...
        pery = abs((y_end-y_init)./y_init) ;
        perx = abs((q_end-q_init)./q_init) ;
        perT = abs((T_end-T_init)./T_init) ;
        
        cyclic_display = [max(pery), max(perx), max(perT), mass_balance] ;
    %   
    end
...
%% Press and Depress Termination functions
    ...
    %PressurizationStop: Function used with ode23t to stop the step
    ...
    %CnCDepressurizationStop: Function used with ode23t to stop the step
