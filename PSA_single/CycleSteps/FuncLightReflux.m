function derivatives = FuncLightReflux(~, state_vars, params, isotherm_params)
%#codegen
%LightReflux: Calculate the change in state variables for Light Reflux step
...
%   
%% Boundary Conditions
%   The boundary condtions for the light reflux, corresponding to both ends 
%   (light and heavy product end) of the column being opened with gas being 
%   fed at a certain molar flow rate through the light product end. For the 
%   heavy product end, the following boundary condtions are used:
%   
%%  
%   $$ \bar{P} = \frac{P_L}{P_{0}} \quad | Z=0^- $$
%   
...
%   These boundary conditions are based off of the light product produced 
%   during the adsorption step and the reflux ratio.
%   
%%  
%   $$ \bar{P}\cdot\bar{v}  = constant \quad | Z=1^+ $$
%   
%   $$ y = y_{LP} \quad | Z=1^+ $$
%   
%   $$ \bar{T} = \bar{T}_{LP} \quad | Z=1^+ $$
%   
%%  
    y(N+2) = y_LR     ;
    T(N+2) = T_LR/T_0 ;
    
%   Calculate the velocity and the inlet pressure of the light product gas
    
    vh = zeros(N+1, 1) ;
    
    MW=MW_N2+(MW_CO2-MW_N2)*y(N+2);
    
    a_1   = 150*mu*(1-epsilon)^2*dz*L/2/4/r_p^2/epsilon^3/T(N+2)/T_0/R ;
    a_2_1 = 1.75*(1-epsilon)/2/r_p/epsilon/epsilon/epsilon*dz*L/2      ;
    a_2   = a_2_1/R/T(N+2)/T_0*ndot_LP*MW                              ;
    
    a =  a_1+a_2                 ;
    b =  P(N+1)/T(N+2)*P_0/R/T_0 ;
    c = -ndot_LP                 ;
    
    vh(N+1) = (-b+sqrt(b^2-4*a*c))/2/a/v_0 ;
    
    a_p = a_1*T(N+2)*T_0*R      ;
    b_p = a_2_1*MW/R/T(N+2)/T_0 ;
    
    P(N+2)  = ((a_p*vh(N+1)*v_0+P(N+1)*P_0)./(1-b_p*vh(N+1)*v_0*vh(N+1)*v_0))/P_0 ;
%   
